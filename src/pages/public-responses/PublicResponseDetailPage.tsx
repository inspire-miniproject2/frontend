...
export function PublicResponseDetailPage() {
  ...
  useEffect(() => { let active = true; const id = Number(responseId); if (!Number.isInteger(id) || id <= 0) { setError('잘못된 답변 번호입니다.'); setLoading(false); return }; getPublicResponse(id).then((result) => { if (active) setData(result) }).catch((reason) => { if (active) setError(reason instanceof ApiError ? reason.message : '공개 답변을 불러오지 못했습니다.') }).finally(() => { if (active) setLoading(false) }); return () => { active = false } }, [responseId])
