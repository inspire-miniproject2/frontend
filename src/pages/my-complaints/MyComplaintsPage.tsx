...
export function MyComplaintsPage() {
  ...
  return <><PageHeader title="내 민원" description="신청한 민원의 배정과 처리 상태를 확인합니다." crumbs={['홈', '내 민원']} action={<Button as={Link} to="/complaints/new/write">민원 신청</Button>} />
    ...
    <form className="filter-bar" onSubmit={submit}>
      <FormField id="keyword" label="민원 제목 또는 민원번호"><TextInput id="keyword" name="keyword" maxLength={50} placeholder="검색어를 입력하세요" /></FormField>
      <FormField id="categoryCode" label="민원 분야">
        <SelectInput id="categoryCode" name="categoryCode" disabled={categoriesLoading || Boolean(categoryError)}>
          <option value="">{categoriesLoading ? '불러오는 중' : '전체'}</option>
          {categories.map((category) => <option key={category.categoryId} value={category.categoryCode}>{category.categoryName}</option>)}
        </SelectInput></FormField>
      <FormField id="status" label="처리 상태"><SelectInput id="status" name="status" defaultValue={query.status ?? ''}>
        <option value="">전체</option><option value="RECEIVED">접수됨</option><option value="ASSIGNED">배정됨</option><option value="IN_PROGRESS">처리 중</option><option value="COMPLETED">완료</option></SelectInput></FormField>
