...
type MockPublicResponseDetail = PublicResponseDetail & { responseId: number }
type MockPublicResponseListItem = PublicResponseListItem & { categoryCode: string }

export const publicResponseDetails: MockPublicResponseDetail[] = [
...
const categoryCodes = ['TRAFFIC', 'ENVIRONMENT', 'WELFARE']
export const publicResponses: MockPublicResponseListItem[] = publicResponseDetails.map((item, index) => ({ responseId: item.responseId, categoryCode: categoryCodes[index], title: item.caseTitle, departmentName: item.departmentName, completedAt: item.completedAt, statusLabel: '답변 완료' }))
