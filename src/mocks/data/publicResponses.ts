...
export const publicResponses: PublicResponseListItem[] = publicResponseDetails.map((item, index) => ({ responseId: item.responseId, categoryCode: categoryCodes[index], categoryName: item.categoryName, title: item.caseTitle, departmentName: item.departmentName, completedAt: item.completedAt, statusLabel: '답변 완료' }))
