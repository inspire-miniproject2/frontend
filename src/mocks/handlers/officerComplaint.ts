...
const categoryNames: Record<string, string> = { TRAFFIC: '도로·교통', ENVIRONMENT: '환경', WELFARE: '복지' }
...
export const officerComplaintHandlers = [
  http.get(`${apiUrl}/officer/complaints`, async ({ request }) => {
    ...
    const content = filtered.slice(page * size, (page + 1) * size).map((item) => ({ complaintId: item.complaintId, complaintNo: item.complaintNo, title: item.title, categoryName: categoryNames[item.categoryCode] ?? item.categoryCode, assigneeName: assignees[item.complaintId] ?? '미지정', submittedAt: item.submittedAt, status: item.currentStatus }))
