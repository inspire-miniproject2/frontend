...
export const officerComplaintHandlers = [
  http.get(`${apiUrl}/officer/complaints`, async ({ request }) => {
    ...
    const content = filtered.slice(page * size, (page + 1) * size).map((item) => ({ complaintId: item.complaintId, complaintNo: item.complaintNo, title: item.title, assigneeName: assignees[item.complaintId] ?? '미지정', assigneeUserId: item.assignedOfficerUserId, status: item.currentStatus, assignedDepartmentId: item.assignedDepartmentId }))
    return HttpResponse.json({ success: true, data: { summary: { newAssigned: count('ASSIGNED'), inProgress: count('IN_PROGRESS'), completed: count('COMPLETED') }, content, page, size, totalElements: filtered.length, totalPages: Math.ceil(filtered.length / size), hasNext: (page + 1) * size < filtered.length }, message: '담당 민원 목록을 조회했습니다.' })
  }),
  http.patch(`${apiUrl}/officer/complaints/:complaintId/status`, async ({ params, request }) => {
    ...
    const previousStatus = complaint.currentStatus; const changedAt = new Date().toISOString(); complaint.currentStatus = newStatus; complaint.statusHistories.push({ previousStatus, newStatus, changedByUserId: 201, changeMemo: body.changeMemo ?? null, changedAt })
  ...
  http.post(`${apiUrl}/officer/complaints/:complaintId/response`, async ({ params, request }) => {
    ...
    const respondedAt = new Date().toISOString(); complaint.response = { responseId: 7100 + complaint.complaintId, responderUserId: 201, isPublic: body.isPublic ?? false, responseContent, respondedAt }
